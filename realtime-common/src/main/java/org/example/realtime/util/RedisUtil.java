...
public class RedisUtil {
    ...
    // Lettuce RedisClient 单例，全局共享，避免创建过多 HashedWheelTimer
    ...
    /**
     * 获取共享的 RedisClient（单例）
     */
    private static RedisClient getRedisClient() {
        if (redisClient == null) {
            synchronized (RedisUtil.class) {
                if (redisClient == null) {
                    redisClient = RedisClient.create("redis://" + Constant.REDIS_HOST + ":" + Constant.REDIS_PORT + "/2");
                }
            }
        }
        return redisClient;
    ...
    public static void writeDimAsync(StatefulRedisConnection<String, String> redisAsyncConn,
        ...
        asyncCommand.setex(key, Constant.REDIS_TWO_DAY_SECONDS, dim.toJSONString());
    ...
    public static void writeDim(Jedis jedis, String tableName, String id, JSONObject dim) {
        ...
        jedis.setex(getKey(tableName, id), Constant.REDIS_TWO_DAY_SECONDS, dim.toJSONString());
    }
