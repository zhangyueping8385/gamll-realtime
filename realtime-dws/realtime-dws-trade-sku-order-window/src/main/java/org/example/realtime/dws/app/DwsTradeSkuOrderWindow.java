...
public class DwsTradeSkuOrderWindow extends BaseAPP {
    public static void main(String[] args) {
        new DwsTradeSkuOrderWindow().Start(10029,4, Constant.DORIS_DWS_TRADE_SKU_ORDER_WINDOW_SYNC_CACHE,Constant.TOPIC_DWD_TRADE_ORDER_DETAIL);
    }
...
//        keyedAndWindowReduceStream.print();
        ...
        //7.维度关联，补充与分组无关的维度字段，业务中一定是先聚合再进行纬度关联
        SingleOutputStreamOperator<TradeSkuOrderBean> mapedStream = getMapedStream(keyedAndWindowReduceStream);

        //8.格式转换。
        SingleOutputStreamOperator<String> mappedStream = mapedStream.map(new DorisMapFunction<>());
        mappedStream.print();
